      ...
      {messages.map((m) =>
        m.from === "bot" ? (
          <div key={m.id} className="flex items-end gap-2">
            ...
            <div className="max-w-[70%] rounded-2xl rounded-bl-sm bg-[var(--surface-1)] px-4 py-2.5 text-[13px] text-[var(--text-primary)]">
              {m.text}
            </div>
          </div>
        ) : (
          <div key={m.id} className="flex justify-end">
            <div className="max-w-[70%] rounded-2xl rounded-br-sm bg-[var(--cta)] px-4 py-2.5 text-[13px] text-white">
              {m.text}
            </div>
          </div>
        )
...
const BIZ_TYPE_QUESTION = "사업자 정보를 선택해주세요.";
...
function Step1Chat({ onComplete }: { onComplete: () => void }) {
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: "bot-bizType", from: "bot", text: BIZ_TYPE_QUESTION },
  ]);
  ...
  const handleSelect = (option: { label: string; value: string }) => {
    if (answered) return;
    setAnswered(true);
    setMessages((prev) => [...prev, { id: "user-bizType", from: "user", text: option.label }]);
    onComplete();
  };
...
export default function SignupTabletScreen() {
